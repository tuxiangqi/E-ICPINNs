...
@physicsnemo.sym.main(config_path="conf", config_name="conf")
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    model_path = to_absolute_path("outputs/2Regularization_pretraining/flow_network.0.pth")
    ...
    # # 5ms inferencer
    # modsim_var = csv_to_dict(to_absolute_path("FSI_3Dcylinder_2NN/SimVascular/data_5ms_validator_xyz0.csv"))
    # modsim_invar = {key: value for key, value in modsim_var.items() if key in ["x", "y", "z", "t"]}
    # modsim_invar = nondimension_invar(modsim_invar, L_scale, t_scale)
    # modsim_invar_inf = normalize_invar(modsim_invar, center, scale, dims=3)
    # grid_inference = PointwiseInferencer(
    #     nodes=nodes,
    #     invar=modsim_invar_inf,
    #     output_names=[
    #         "u",
    #         "v",
    #         "w",
    ...
    #     requires_grad=True,
    # )
    # domain_flow.add_inferencer(grid_inference, "times_5ms_inf_data")
    # print("accomplish times_5ms grid inferencer")

    # # 2.5ms inferencer
    # modsim_var = csv_to_dict(to_absolute_path("FSI_3Dcylinder_2NN/SimVascular/data_2.5ms_validator_xyz0.csv"))
    # modsim_invar = {key: value for key, value in modsim_var.items() if key in ["x", "y", "z", "t"]}
    # modsim_invar = nondimension_invar(modsim_invar, L_scale, t_scale)
    # modsim_invar_inf = normalize_invar(modsim_invar, center, scale, dims=3)
    # grid_inference_2_5ms = PointwiseInferencer(
    #     nodes=nodes,
    #     invar=modsim_invar_inf,
    #     output_names=[
    #         "u",
    ...
    #     requires_grad=True,
    # )
    # domain_flow.add_inferencer(grid_inference_2_5ms, "times_2.5ms_inf_data")
    # print("accomplish times_2.5ms grid inferencer")

    # ======================================================
    # Alternating training: 10 cycles
    # Each cycle: displacement for 1000 steps, then flow for 2500 steps
    # ======================================================
    cfg_disp = deepcopy(cfg)
    cfg_flow = deepcopy(cfg)

    per_cycle_disp = 10000   # number of training steps for the displacement net per cycle
    per_cycle_flow = 25000   # number of training steps for the flow net per cycle
    per_cycle_total = per_cycle_disp + per_cycle_flow

    num_cycles = 10
    start_time_outer = time.time()

    slv_disp = Solver(cfg_disp, domain_disp)
    slv_flow = Solver(cfg_flow, domain_flow)

    # record the "global step at the end of the previous cycle", starting from 0
    global_target = 0

    for cycle in range(num_cycles):
        print(f"\n======== Cycle {cycle+1}/{num_cycles}: train displacement_net (eta) ========")

        # target global step for the displacement stage in this cycle
        disp_target = global_target + per_cycle_disp
        # key: modify slv_disp.max_steps (or the internal trainer's max_steps)
        slv_disp.max_steps = disp_target  # not slv_disp.cfg.training.max_steps

        # update displacement_net only
        ...
        print(f"\n======== Cycle {cycle+1}/{num_cycles}: train flow_net (u,v,w,p) ========")

        # target global step for the flow stage in this cycle
        flow_target = global_target + per_cycle_disp + per_cycle_flow
        slv_flow.max_steps = flow_target
        ...
        slv_flow.solve()
        # after one cycle, advance global_target by one full cycle
        global_target += per_cycle_total
