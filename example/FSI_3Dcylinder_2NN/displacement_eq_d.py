...
class Epsilon_nondim(PDE):
    ...
    where:
      ...
      Here we adopt axisymmetry + radial-dominant deformation, using
        R/R0 = 1 + (L η*) / R0
        g ≈ R/R0                      (for a more refined form, you may change it to
                                       R/R0*sqrt(1 + (∂η/∂z)^2))
    """
    ...
    def __init__(self,
        ...
        # ---- physical scale parameters (Number is convenient for sympy handling) ----
        rho_f = Number(rho_f)
        V     = Number(V)

        rho_w = Number(1200.0)     # vessel wall density ρ_w
        nu_w  = Number(0.3)        # Poisson ratio ξ
        E_w   = Number(3.0e5)      # Young's modulus E
        ...
        # ---- displacement network outputs: d_x*, d_y*, d_z* ----
        dx = Function("dx")(*input_variables)
        dy = Function("dy")(*input_variables)
        dz = Function("dz")(*input_variables)
        ...
        # ----------------- compute η* from d_x, d_y, d_z -----------------
        # η* = d* · e_r
        eta = dx * e_rx + dy * e_ry + dz * e_rz   # this is the non-dimensional η*
        ...
        #g = R_over_R0
        # for a more refined form, you may change it to:
        eta_z = eta.diff(z)
        g = R_over_R0 * sqrt(1 + eta_z**2)
        

        # ----------------- non-dimensional coefficients b* and H* -----------------
...
class harmonic_extension(PDE):
    def __init__(self):
        # coordinates and time (already non-dimensional)
        x, y, z = Symbol("x"), Symbol("y"), Symbol("z")
        t       = Symbol("t")

        input_variables = {"x": x, "y": y, "z": z, "t": t}

        # now treat the displacement vector d = (dx, dy, dz) directly as the unknown
        dx = Function("dx")(*input_variables)
        dy = Function("dy")(*input_variables)
        dz = Function("dz")(*input_variables)

        # standard 3D Laplacian operator
        def lap(f):
            return f.diff(x, 2) + f.diff(y, 2) + f.diff(z, 2)

        self.equations = {}
        # enforce Δ d_i = 0 for each component
        self.equations["harmonic_x"] = simplify(lap(dx))
        self.equations["harmonic_y"] = simplify(lap(dy))
        # if you also want harmonic extension in the z direction, add "harmonic_z": 0 in the constraint
        self.equations["harmonic_z"] = simplify(lap(dz))
