...
class LRstepwise(Aggregator):
    """
    Simplified LRstepwise: applies a stepwise (staircase) amplification to a certain
    class of losses based on the training step:

        lambda(step) = base_weight * growth_factor ** floor(step / step_interval)
    ...
    Then:
        step in [0,1999]    -> 1e-8
        step in [2000,3999] -> 1e-7
        step in [4000,5999] -> 1e-6
        ...
    """
    ...
    def __init__(
        ...
        params,
        num_losses,
        update_freq: int = 1,   # keep the old interface (actually no longer used)
        alpha: float = 0.01,
        ref_key=None,
        eps: float = 1e-8,
        weights=None,
        # one-to-one mapping with newly added fields in AggregatorLRAnnealingConf
        base_weight: float = 1.0e-8,
        growth_factor: float = 10.0,
        step_interval: int = 2000,
        pde_key: str | None = "interior",
    ):
        super().__init__(params, num_losses, weights)
        ...
        # by default, set lambda=1 for all losses
        self.register_buffer(
            "lmbda_ema", torch.ones(self.num_losses, device=self.device)
        )

    def _need_schedule(self, key: str) -> bool:
        # pde_key=None means applying this schedule to all losses
        if self.pde_key is None:
            return True
        return self.pde_key in key

    def _update_lambdas(self, losses: Dict[str, torch.Tensor], step: int):
        if self.step_interval <= 0:
            return

        # at step 0, set scheduled terms to base_weight
        if step == 0:
            for i, key in enumerate(losses.keys()):
                if self._need_schedule(key):
                    self.lmbda_ema[i] = self.base_weight

        # at every multiple of step_interval, multiply by growth_factor
        elif step % self.step_interval == 0:
            for i, key in enumerate(losses.keys()):
                if self._need_schedule(key):
                    self.lmbda_ema[i] = self.lmbda_ema[i] * self.growth_factor

    def forward(self, losses: Dict[str, torch.Tensor], step: int) -> torch.Tensor:
        # first apply static weights
        losses = self.weigh_losses(losses, self.weights)

        # update lambda according to the current step
        self._update_lambdas(losses, step)

        # aggregate
        loss = torch.zeros_like(self.init_loss)
        for i, key in enumerate(losses.keys()):
            loss += self.lmbda_ema[i] * losses[key]
        return loss
